# ⚡ Charlie_Recon — Product Requirements Document (PRD)

> **Tool Name:** Charlie_Recon  
> **Author:** ImrulMir  
> **Version:** 1.0.0  
...
Charlie_Recon is a production-ready, modular, CLI-based automated reconnaissance and vulnerability scanning tool designed for penetration testers and security researchers. It accepts a domain, subdomain, URL, or IP address and performs a comprehensive security assessment — from passive/active recon to vulnerability scanning — then generates a stunning cyberpunk-themed HTML report.
...
```mermaid
graph TB
    A["CLI Entry Point<br/>charlie_recon.py"] --> B["Core Engine<br/>engine.py"]
    B --> C["Input Validator<br/>core/validator.py"]
...
|--------|------|-------------|
| **CLI Interface** | `charlie_recon.py` | argparse-based entry point, banner display, orchestration |
...
| Module | File | Format |
|--------|------|--------|
| **HTML Report** | `modules/report/html_report.py` | Cyberpunk-themed HTML with charts, severity badges |
...
```
usage: charlie_recon.py [-h] -t TARGET [-o OUTPUT] [--threads THREADS]
...
⚡ Charlie_Recon v1.0.0 — Automated Recon & Vulnerability Scanner by ImrulMir

Required:
  -t, --target TARGET     Target domain, URL, or IP address

Options:
  -o, --output OUTPUT     Output directory (default: ./charlie_recon_output)
...
### Theme: "Cyberpunk Dashboard" — Dark hacker aesthetic

- **Color Palette**: 
  - Background: `#0d1117` (deep dark)
  - Primary: `#00ff88` (neon green)
  - Accent: `#ff2d55` (neon red for critical)
  - Secondary: `#00e5ff` (cyber cyan)
  - Warning: `#ffc107` (amber)
- **Typography**: JetBrains Mono (monospace), Orbitron (headings)
- **Features**:
  - Animated scan lines with fade effect
  ...
  - Summary dashboard with statistics cards (hover lift + glow)
  - Donut chart for vulnerability severity distribution
  - Responsive design with styled scrollbars
  - Print-friendly mode
...
| ✅ HTML report | Full cyberpunk-themed report with charts |
...
```
Charlie_Recon/
├── charlie_recon.py              # Main entry point
...
│   └── seclists.py               # SecLists integration
├── modules/
│   ├── __init__.py
│   ├── recon/
│   │   ├── __init__.py
...
│       ├── html_report.py
│       └── json_export.py
└── charlie_recon_output/         # Default output directory
```
